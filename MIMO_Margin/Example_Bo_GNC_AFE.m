...
rr=Zo_vsi_vil*Yin_vil_pll_cal_frd_s;
RRdd=rr(1,1);
RRdq=rr(1,2);
RRqd=rr(2,1);
RRqq=rr(2,2);
...
% closed loop transfer function response
...
%   Lqqeigenvalues(:,k)=eig(Lqqresp(:,:,k));
    Leigenvalues(:,k)=eig(RR_res(:,:,k));           % find the eigenvalue of the return ratio
    [V,D]=eig(RR_res(:,:,k));                       % find the eigenvector of the return ratio
    C_V(k)=dot(V(:,1),V(:,2));                      % dot product of eigenvectors
    Singular_L(:,k) = svd(RR_res(:,:,k));           % find the singular value of the return ratio
end
...
%         Lqqeigenvalues(:,k)=eig(Lqqresp(:,:,k));
    Leigenvalues(:,k)=eig(RR_res(:,:,k));
    [V,D]=eig(RR_res(:,:,k));                       % find eigenvalues and eigenvectors
    C_V(k)=dot(V(:,1),V(:,2));                      % dot product of eigenvectors
    Singular_L(:,k) = svd(RR_res(:,:,k));           % find the singular value of return ratio matirx
end
% sort eigenvalue loci
[Leigenvalues]=sortloci(Leigenvalues);
% plot singular value and abs of eigenvalue
figure(12)
semilogx(freq_test(1:length(freq_test)*k_freq_r),Singular_L(1,:),'b*');
...
semilogx(freq_test(1:length(freq_test)*k_freq_r),abs(Leigenvalues(2,:)),'k');
grid on
% plot the dot product of eigenvectors of the return ratio, 
figure(62)
plot(C_V)
grid on
