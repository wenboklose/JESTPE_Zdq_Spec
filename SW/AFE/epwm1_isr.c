...
double counter_PLL_Max			= 2000;//200000;
...
// for VSI from sensor 1
	...
	Vdc_AFE		= u10;
    // set as Id reference signal for AFE
    Per         = 0*u11;
...
//     O_loop_VSI();
    ...
    {
        V_loop_AFE();
        iqref_AFE = 0.0*idref_AFE;//0.9*idref_AFE;//-0.9*idref_AFE;//
// 			idref_AFE = Per;      //7.88
        I_loop_AFE();
        *y7 = 1;
    ...
    *y8=Id_AFE;//FW_VLL[0];//Dd_out_VSI;//Id_VSI+Per;
    *y9=VLL_d_AFE;//VLL_DQ_theta_AFE;//Iq_AFE;//Id_VSI;
}
...
//     K_decoup_VSI            = 1;
	...
	Vd_out_VSI				= K_decoup_VSI*Two_PI*f_line*1*Lout_VSI*Iq_VSI/Vdc_VSI+Kpi_VSI*(err_id)+In_ud_VSI;		
	Vq_out_VSI				= -K_decoup_VSI*Two_PI*f_line*1*Lout_VSI*Id_VSI/Vdc_VSI+Kpi_VSI*(err_iq)+In_uq_VSI;
...
// 	}
	...
	Vd_out_AFE				= K_decoup_AFE*Two_PI*f_line*0.8*Lout_AFE*Iq_AFE+Kpi_AFE*(idslop_AFE-Id_AFE)+In_ud_AFE;		
	Vq_out_AFE				= -K_decoup_AFE*Two_PI*f_line*0.8*Lout_AFE*Id_AFE+Kpi_AFE*(iqslop_AFE-Iq_AFE)+In_uq_AFE;
