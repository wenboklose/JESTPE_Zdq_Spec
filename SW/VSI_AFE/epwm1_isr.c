...
double f_line                   = 60;//400;					//line frequency (Hz) for ac bus
...
double Kpv_VSI					= 0;
double Kiv_VSI					= 0;
double K_decoup_VSI				= -1;
double K_decoup_VSI_v			= -1;
...
double 		Vdc_AFE_step		= 1;
double Kpi_AFE					= 0;
double Kii_AFE					= 0;
double Kpv_AFE					= 0;
double Kiv_AFE					= 0;
double K_decoup_AFE				= -1;
...
// variables for pll
double DEF_pll =1;                                      //control loop for PLL (Hz)
...
void epwm1_isr(double *y1, double *y2, double *y3, double *y4, double *y5, double *y6, double *y7, double *y8, double *y9, double u1, double u2, double u3, double u4, double u5, double u6, double u7, double u8, double u9, double u10, double u11, double u12, double u13, double u14, double u15, double u16, double u17, double u18, double u19, double u20, double u21)
{
...
//	output_vector_angle_2 	= 0;					//reference voltage angle
	...
	f_line					= 60;//400;					//line frequency (Hz)
...
void V_loop_VSI(void)
{
	...
	idref_VSI=K_decoup_VSI*Two_PI*f_line*Cout_VSI*Vq_VSI+Kpv_VSI*(err_vd)+In_idref_VSI;		
	iqref_VSI=-K_decoup_VSI*Two_PI*f_line*Cout_VSI*Vd_VSI+Kpv_VSI*(err_vq)+In_iqref_VSI;
}
...
void I_loop_VSI(void)
{
	...
	Ma_2					= out_vector_alpha_VSI*SQRT_3over2/Vdc_VSI;//270;//							// Valpha*sqrt(3/2)/Vdc
	Mk_2					= out_vector_beta_VSI*SQRT_1over2/Vdc_VSI;//270;//							// Vbeta*sqrt(3/2)/Vdc/sqrt(3)
...
void I_loop_AFE(void)
{
	...
	Ma_1					= out_vector_alpha_AFE*SQRT_3over2/Vdc_AFE;//270;//							// Valpha*sqrt(3/2)/Vdc
	Mk_1					= out_vector_beta_AFE*SQRT_1over2/Vdc_AFE;//270;//							// Vbeta*sqrt(3/2)/Vdc/sqrt(3)
}
void V_loop_AFE(void)
{
	...
	idref_AFE=((vdcslop_AFE-Vdc_AFE)*Kpv_AFE+In_idref_AFE)*270*One_Vpd;//Vdc_AFE*One_Vpd;//
