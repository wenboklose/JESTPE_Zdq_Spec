...
for i=1:1:1
    Id_ref = 1*id_ref(2);
    Kp1 = 0.1;%0.1; %No.1 PLL Kp (inverter)
    Ki1 = Ki_pll(1); %2; %No.1 PLL Ki 3.2 is unstable
    tf_pll = Kp1+Ki1/s;
...
%     Gsvm = [Gsvmdd*cos(omega*Tdelay) 0*Gsvmdd*sin(omega*Tdelay);-0*Gsvmdd*sin(omega*Tdelay) Gsvmdd*cos(omega*Tdelay)];
    ...
    Ki = tf_filter_dq;
    Kv = tf_filter_dq;%Hv;
    sys = linearize('VSI_GT_avg_il_pll',0.5);
...
%     bode(Zin_il_pll_avg_cal_frd,Zin_il_pll_avg_sim,Bode_O)
    ...
    set(fig, 'Position', [50, 10, 1000, 800]);
%     bode(Zin_il_pll_avg_cal_frd,Zin_il_pll_avg_sim,Bode_O)
    bode(Zin_il_pll_avg_sim(1,1),Zin_il_pll_avg_sim(1,2),Zin_il_pll_avg_sim(2,1),Zin_il_pll_avg_sim(2,2),Bode_O)
    hold on
    Bode_Darklines(3)

    Yin_il_pll_avg_sim = inv(Zin_il_pll_avg_sim);
    fig=figure(8)
    set(fig, 'Position', [50, 10, 1000, 800]);
    bode(Yin_il_pll_avg_sim(1,1),Yin_il_pll_avg_sim(1,2),Yin_il_pll_avg_sim(2,1),Yin_il_pll_avg_sim(2,2),Bode_O)
    hold on
    Bode_Darklines(3)
